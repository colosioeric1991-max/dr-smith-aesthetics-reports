...
// ── Editorial portrait for slide 1 (ChatGPT-generated, 4:5).
const imgPath = resolve(__dirname, 'carousel-images/ageing.png');
let imgBase64 = '';
try {
  const buf = await readFile(imgPath);
  imgBase64 = 'data:image/png;base64,' + buf.toString('base64');
} catch {
  console.log('⚠  Portrait not found – slide 1 will show a placeholder');
}
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ══════════════════════════════════════════════════════════
     SLIDE 1 – FULL-BLEED PHOTO REVEAL (photo background, text overlaid)
  ══════════════════════════════════════════════════════════ */
  #s1 { background: var(--teal); }
  .s1-bg { position: absolute; inset: 0; }
  .s1-bg img { width: 100%; height: 100%; object-fit: cover; object-position: 62% 20%; }
  .s1-wash {
    position: absolute; inset: 0;
    background: linear-gradient(180deg,
      rgba(13,59,79,0.80) 0%,
      rgba(13,59,79,0.38) 32%,
      rgba(13,59,79,0.30) 58%,
      rgba(13,59,79,0.82) 100%);
  }
  .s1-text {
    position: absolute;
    top: 72px; left: 76px; right: 76px;
  }
  .s1-text .eyebrow { margin-bottom: 24px; }
  .s1-title {
    font-family: 'Cormorant Garamond', serif;
    font-size: 6.4rem; font-weight: 300; line-height: 0.98;
    letter-spacing: -0.03em; color: var(--warm); margin-bottom: 26px;
  }
  .s1-title em { font-style: italic; font-weight: 400; color: rgba(201,169,110,0.95); }
  .s1-rule { width: 60px; height: 1px; background: rgba(201,169,110,0.5); margin-bottom: 26px; }
  .s1-sub {
    font-family: 'DM Sans', sans-serif; font-size: 2.1rem; font-weight: 300;
    line-height: 1.6; letter-spacing: 0.03em; color: rgba(253,250,246,0.78);
    max-width: 760px;
  }
...
<!-- ══ SLIDE 1 : REVEAL ════════════════════════════════════ -->
<div class="slide active bg-teal" id="s1">
  <div class="s1-bg">
    ${imgBase64 ? `<img src="${imgBase64}" alt="Elegant mature skin" />` : ''}
  </div>
  <div class="s1-wash"></div>
  <div class="s1-text">
    <p class="eyebrow">The Science of Ageing · Dr Smith Aesthetics</p>
    <h1 class="s1-title">What <em>actually</em><br>ages your face</h1>
    <div class="s1-rule"></div>
    <p class="s1-sub">It's not just wrinkles. Here's what's really happening underneath.</p>
  </div>
  <div class="progress"><div class="progress-fill" style="width:20%"></div></div>
  <div class="footer">
    <span class="f-left">The Ageing Face · Education</span>
...
<div class="slide bg-teal" id="s2">
  ...
  <div class="s2-content">
    ...
    <h2 class="s2-headline">
      Wrinkles are the symptom.<br>
      <em>Not the cause.</em>
    </h2>
    <div class="s2-rule"></div>
    <p class="s2-body">From our mid-twenties, the face changes in layers: bone, fat, muscle and skin. Understanding what's actually happening underneath is the difference between chasing lines and treating the cause.</p>
  </div>
...
<div class="slide bg-cream" id="s3">
  <div class="s3-content">
    <p class="eyebrow">The Anatomy</p>
    <h2 class="s3-headline">What <em>changes</em></h2>
    <div class="s3-rule"></div>
    <div class="s3-items">
      <div class="s3-item">
        <div class="s3-num">01</div>
        <div>
          <p class="s3-label">Bone Resorption</p>
          <p class="s3-text">The facial skeleton slowly loses volume, weakening the scaffold that supports everything above it.</p>
        </div>
      </div>
      <div class="s3-item">
        <div class="s3-num">02</div>
        <div>
          <p class="s3-label">Fat Pad Loss</p>
          <p class="s3-text">Deep facial fat pads shrink and drift downwards, hollowing the cheeks and temples.</p>
        </div>
      </div>
      <div class="s3-item">
        <div class="s3-num">03</div>
        <div>
          <p class="s3-label">Collagen Decline</p>
          <p class="s3-text">Production falls by roughly 1% a year, leaving skin thinner and less elastic.</p>
        </div>
      </div>
      <div class="s3-item">
        <div class="s3-num">04</div>
        <div>
          <p class="s3-label">Sun &amp; Lifestyle</p>
          <p class="s3-text">UV exposure drives the majority of visible skin ageing, well ahead of time itself.</p>
        </div>
      </div>
    </div>
  </div>
...
<!-- ══ SLIDE 4 : WHAT YOU CAN DO ══════════════════════════════ -->
<div class="slide bg-teal" id="s4">
  <div class="s4-glow"></div>
  <div class="s4-content">
    <p class="eyebrow">The Response</p>
    <h2 class="s4-headline">What you can <em>do</em></h2>
    <div class="s4-rule"></div>
    <div class="s4-items">
      <div class="s4-item">
        <div class="s4-num">01</div>
        <div>
          <p class="s4-label">Protect</p>
          <p class="s4-text">Daily SPF is still the single most effective anti-ageing step there is.</p>
        </div>
      </div>
      <div class="s4-item">
        <div class="s4-num">02</div>
        <div>
          <p class="s4-label">Rebuild</p>
          <p class="s4-text">Biostimulators like Sculptra restore the deep structural support the face loses.</p>
        </div>
      </div>
      <div class="s4-item">
        <div class="s4-num">03</div>
        <div>
          <p class="s4-label">Regenerate</p>
          <p class="s4-text">Skin boosters and polynucleotides improve the quality of the skin itself.</p>
        </div>
      </div>
      <div class="s4-item">
        <div class="s4-num">04</div>
        <div>
          <p class="s4-label">Refine</p>
          <p class="s4-text">Targeted Botox and filler then treat what remains, in proportion.</p>
        </div>
      </div>
    </div>
  </div>
...
<div class="slide bg-cream" id="s5">
  <div class="s5-content">
    <p class="eyebrow centered">The Science of Ageing</p>
    <h2 class="s5-headline">Treat the <em>cause</em>,<br>not just the lines.</h2>
    <div class="s5-divider"></div>
    <p class="s5-sessions">Full-face consultations available now</p>
...
const outputDir = resolve(__dirname, '../Carousels/Ageing-Carousel');
...
const slides = [
  ...
  { id: 's3', file: '03-what-changes.png' },
  { id: 's4', file: '04-what-you-can-do.png' },
